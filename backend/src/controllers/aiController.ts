...
function localFormat(text: string, title?: string, template = 'beautify'): string {
  const clean = stripHtml(text);
...
async function callGeminiApi(prompt: string, apiKey: string, maxTokens = 1500): Promise<string | null> {
  try {
...
กรุณาตอบเฉพาะข้อความที่เรียบเรียงเสร็จแล้วเป็นภาษาไทย โดยคงความหมายเดิมไว้ ไม่ต้องมีคำเกริ่นนำหรือคำลงท้าย:`;
  ...
  static async format(req: AuthRequest, res: Response) {
    try {
      ...
      const templateInstructions: Record<string, { name: string; desc: string }> = {
        beautify: {
