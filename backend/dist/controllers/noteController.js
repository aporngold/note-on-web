...
const createNoteSchema = zod_1.z.object({
    ...
    color: zod_1.z.string().optional().default('#FEF08A'),
    textColor: zod_1.z.string().optional().default('#1e293b'),
...
class NoteController {
    ...
    static async createNote(req, res) {
        try {
            ...
            const newNote = await database_1.prisma.note.create({
                data: {
    ...
    static async duplicateNote(req, res) {
        try {
            const userId = req.userId;
            const { id } = req.params;
            const original = await database_1.prisma.note.findFirst({
                where: { id, userId },
                include: { labels: true },
            });
            if (!original) {
    ...
    static async exportBackup(req, res) {
        try {
            ...
            const [boards, notebooks, labels, notes, connections] = await Promise.all([
                database_1.prisma.board.findMany({ where: { userId } }),
                database_1.prisma.notebook.findMany({ where: { userId } }),
                database_1.prisma.label.findMany({ where: { userId } }),
                database_1.prisma.note.findMany({
                    where: { userId },
                    include: {
                        attachments: true,
                ...
                database_1.prisma.noteConnection.findMany({
                    where: {
                        board: { userId },
                    },
                }),
