...
const notebookSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, 'Notebook name is required').max(50),
...
class NotebookController {
    static async getNotebooks(req, res) {
        try {
            ...
            return res.json(notebooks.map((nb) => ({
                id: nb.id,
                name: nb.name,
    ...
    static async createNotebook(req, res) {
        try {
            ...
            const notebook = await database_1.prisma.notebook.create({
                data: {
                    name,
    ...
    static async updateNotebook(req, res) {
        try {
            ...
            const notebook = await database_1.prisma.notebook.findFirst({
                where: { id, userId },
            });
            if (!notebook) {
                return res.status(404).json({ error: 'Notebook not found' });
            }
            const updated = await database_1.prisma.notebook.update({
                where: { id },
                data: parsed.data,
            });
    ...
    static async deleteNotebook(req, res) {
        try {
            ...
            await database_1.prisma.notebook.delete({ where: { id } });
